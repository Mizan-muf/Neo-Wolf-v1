...
### Checklist
- [ ] Create `Engine` class
- [ ] Create `App` or `GameHost` class
- [ ] Create `PlatformSDL` wrapper
- [ ] Create `InputState` struct
- [ ] Create `TimeState` struct
- [ ] Create `Engine::Init()`
- [ ] Create `Engine::Update()`
- [ ] Create `Engine::Render()`
- [ ] Create `Engine::Shutdown()`
- [ ] Separate startup/update/render/shutdown into different files
- [ ] Keep SDL calls mostly inside platform layer

### Done When
- [ ] Main loop calls engine update and render functions cleanly
- [ ] Project is split into modules, not one giant file
